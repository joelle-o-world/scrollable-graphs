...
divisions = [60, 10, 10, 10, 10], minSpacing = 10, format = formatTime, minLabelSpacing = 50, }) => {
        ...
        for (let j = 0; j < nMarks + 1; ++j) {
            ...
            if (i == 0 || (firstIndex + j) % divisions[i - 1]) {
                ...
                if (spacing > minLabelSpacing) {
                    let label = format(t);
                    markers.push(React.createElement("text", { x: x, style: { textAlign: 'center' }, y: y + 12, key: Math.random() }, label));
                }
            }
        }
        unit /= divisions[i];
    }
    return React.createElement("g", null, markers);
};
